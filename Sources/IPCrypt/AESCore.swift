...
struct AESCore {
    ...
    static func mixColumns(_ state: inout Data) {
        for i in stride(from: 0, to: 16, by: 4) {
            ...
            state[i] = gfMul2(s0) ^ gfMul(s1, 3) ^ s2 ^ s3
            state[i + 1] = s0 ^ gfMul2(s1) ^ gfMul(s2, 3) ^ s3
            state[i + 2] = s0 ^ s1 ^ gfMul2(s2) ^ gfMul(s3, 3)
            state[i + 3] = gfMul(s0, 3) ^ s1 ^ s2 ^ gfMul2(s3)
        }
    }
